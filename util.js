...
const STATS_URL =
	"https://api.teleport.org/api/cities/?embed=city:search-results/city:item/city:urban_area/ua:scores&search=";

async function getJobs(location) {
	try {
		const { data } = await axios.get(`${JOBS_URL}?location=${location}`, {
			headers: {
...
async function getCityInfo(location) {
	try {
		...
		const scores =
			city && city._embedded && city._embedded["city:urban_area"]._embedded["ua:scores"];
